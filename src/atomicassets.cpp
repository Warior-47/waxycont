...
*/
ACTION atomicassets::bulkminting(
    ...
    check(nft_count > 0, "NFT count is not correct");
    for(int32_t i = 0; i<nft_count; i++){
...
void atomicassets::internal_transfer_offer(
    name from,
    name to,
    vector <uint64_t> asset_ids,
    vector <uint64_t> asset_ids_other,
    string memo,
    name scope_payer,
    name scope_payer_other
) {
    ...
    for (uint64_t asset_id : asset_ids_other) {
        auto asset_itr = to_assets.require_find(asset_id,
            ("Sender doesn't own at least one of the provided assets (ID: " +
             to_string(asset_id) + ")").c_str());
        ...
        if (no_previous_scope) {
            ...
            //This asset is later deleted again.
            //This action will therefore fail is the scope_payer didn't authorize the action
            from_assets.emplace(scope_payer_other, [&](auto &_asset) {
                _asset.asset_id = ULLONG_MAX;
                _asset.collection_name = name("");
                _asset.schema_name = name("");
                _asset.template_id = -1;
                _asset.ram_payer = scope_payer_other;
                _asset.backed_tokens = {};
                _asset.immutable_serialized_data = {};
                _asset.mutable_serialized_data = {};
            });
        ...
        from_assets.emplace(asset_itr->ram_payer, [&](auto &_asset) {
            _asset.asset_id = asset_itr->asset_id;
            _asset.collection_name = asset_itr->collection_name;
            _asset.schema_name = asset_itr->schema_name;
            _asset.template_id = asset_itr->template_id;
        ...
        to_assets.erase(asset_itr);

        if (no_previous_scope) {
            from_assets.erase(from_assets.find(ULLONG_MAX));
        }
    }
    //Sending notifications
}
////////////////////////////////////////////////////////////////////////////////////////////
ACTION atomicassets::offery(
         uint64_t offer_id,
         name sender,
         name receiver
) {  
...
////////////////////////// Modified offer exchange internal transfer
